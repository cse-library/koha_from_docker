...
<xsl:stylesheet version="1.0"
 ...
 <xsl:if test="contains($field019b,'ab')"> Bản đồ</xsl:if>
 ...
 <xsl:if test="$controlField007-01='b'">Bản đồ</xsl:if>
 ...
 <xsl:if test="$controlField007-01='c'">Tranh dán</xsl:if> <!-- Originalt kunstverk -->
 ...
 <p class="author">av <xsl:for-each select="marc:datafield[(@tag=100 or @tag=700) and @ind1!='z']">
 <a>
 <xsl:choose>
 ...
 <xsl:when test="$controlField008-26='f'">Phông chữ</xsl:when>
 ...
 <xsl:if test="contains($controlField008-30-31,'d')">Kịch</xsl:if>
 <xsl:if test="contains($controlField008-30-31,'e')">Tiểu luận</xsl:if>
