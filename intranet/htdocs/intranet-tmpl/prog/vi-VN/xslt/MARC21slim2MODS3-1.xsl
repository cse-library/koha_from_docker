...
-->
 ...
 <identifier>
 <xsl:call-template name="isInvalid"/>
 <xsl:attribute name="type"><xsl:choose><xsl:when test="@ind1=0">Số kỳ phát hành</xsl:when><xsl:when test="@ind1=1">matrix number</xsl:when><xsl:when test="@ind1=2">music plate</xsl:when><xsl:when test="@ind1=3">music publisher</xsl:when><xsl:when test="@ind1=4">videorecording identifier</xsl:when></xsl:choose></xsl:attribute>
 <xsl:call-template name="subfieldSelect">
 ...
 <xsl:attribute name="invalid">Có</xsl:attribute>
 </xsl:if>
 </xsl:template>
 ...
 <xsl:attribute name="script"><xsl:choose><xsl:when test="$scriptCode='(3'">Tiếng Ả Rập</xsl:when><xsl:when test="$scriptCode='(B'">Hệ ngôn ngữ Latinh</xsl:when><xsl:when test="$scriptCode='$1'">Chinese, Japanese, Korean</xsl:when><xsl:when test="$scriptCode='(N'">Cyrillic</xsl:when><xsl:when test="$scriptCode='(2'">Hebrew</xsl:when><xsl:when test="$scriptCode='(S'">Greek</xsl:when></xsl:choose></xsl:attribute>
 </xsl:template>
 ...
 <language>
 <xsl:if test="@code!='a'">
 <xsl:attribute name="objectPart"><xsl:choose><xsl:when test="@code='b'">Tóm tắt hoặc nhan đề phụ</xsl:when><xsl:when test="@code='d'">sung or spoken text</xsl:when><xsl:when test="@code='e'">Thể loại nhạc kịch</xsl:when><xsl:when test="@code='f'">table of contents</xsl:when><xsl:when test="@code='g'">accompanying material</xsl:when><xsl:when test="@code='h'">translation</xsl:when></xsl:choose></xsl:attribute>
 </xsl:if>
 <languageTerm authority="iso639-2b" type="code">
