...
-->
 ...
 <xsl:text>Trực tiếp</xsl:text>
 </xsl:attribute>
 </xsl:when>
 <xsl:when test="$controlField008-06='i'">
 <xsl:attribute name="geographicSubdivision">
 <xsl:text>Gián tiếp</xsl:text>
 </xsl:attribute>
 </xsl:when>
 ...
 <mads:recordInfo>
 <mads:recordOrigin>Được chuyển đổi từ MARCXML sang MADS phiên bản 2.0</mads:recordOrigin>
 <!-- <xsl:apply-templates select="marc:datafield[@tag=024]"/> -->
 ...
 <mads:descriptionStandard>
 <xsl:text>Quy tắc khác</xsl:text>
 </mads:descriptionStandard>
 </xsl:if>
 </xsl:for-each>
 </mads:recordInfo>
...
-->
 ...
 <xsl:attribute name="type">muộn</xsl:attribute>
 </xsl:if>
 ...
 <xsl:attribute name="type">Báo cáo khác</xsl:attribute>
 </xsl:if>
 ...
 <xsl:attribute name="type">Từ viết tắt</xsl:attribute>
 </xsl:if>
 ...
 <xsl:attribute name="authority">Tiền mặt</xsl:attribute>
 </xsl:when>
 <xsl:when
